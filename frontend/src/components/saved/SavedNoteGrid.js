...
import { SavedNoteCard, setupSavedNoteCardEvents } from './SavedNoteCard.js';
    ...
    return `
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div class="flex items-center gap-3">
                <div class="bg-[#1F2937] rounded-xl p-1 flex gap-1 border border-[#374151]">
                    <button class="view-toggle-btn p-2 rounded-lg transition-all duration-300 ${viewMode === 'grid' ? 'bg-gradient-to-r from-[#3B82F6] to-[#A78BFA] text-white shadow-md' : 'text-[#9CA3AF] hover:text-[#E5E7EB] hover:bg-[#374151]'}" data-view="grid" title="Grid View">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path>
                        </svg>
                    </button>
                    <button class="view-toggle-btn p-2 rounded-lg transition-all duration-300 ${viewMode === 'list' ? 'bg-gradient-to-r from-[#3B82F6] to-[#A78BFA] text-white shadow-md' : 'text-[#9CA3AF] hover:text-[#E5E7EB] hover:bg-[#374151]'}" data-view="list" title="List View">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button id="exportNotesBtn" class="bg-[#374151] hover:bg-gradient-to-r hover:from-[#3B82F6] hover:to-[#A78BFA] text-[#9CA3AF] hover:text-white px-4 py-2 rounded-xl text-sm transition-all duration-300 flex items-center gap-2 shadow-md hover:shadow-lg">
...
// Setup saved note grid events
export function setupSavedNoteGrid(onView, onDelete, onViewToggle, onExport) {
    setupSavedNoteCardEvents(onView, onDelete);
    ...
    viewToggleBtns.forEach(btn => {
        ...
        newBtn.addEventListener('click', () => {
            const viewMode = newBtn.dataset.view;
            if (onViewToggle) onViewToggle(viewMode);
        });
    });
    ...
    if (exportBtn && onExport) {
        ...
        newExportBtn.addEventListener('click', () => onExport());
    }
}

// Add CSS animations
const savedNoteStyles = `
    @keyframes fadeInUp {
        from {
            opacity: 0;
...
if (!document.querySelector('#saved-note-styles')) {
    const style = document.createElement('style');
    style.id = 'saved-note-styles';
    style.textContent = savedNoteStyles;
