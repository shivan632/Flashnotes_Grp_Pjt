...
export function SavedNoteFilter({ topics, onFilterChange, onSortChange, onSearch }) {
    ...
    return `
        <div class="bg-gradient-to-br from-[#1F2937] to-[#111827] rounded-2xl p-5 border border-[#374151] mb-6">
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div class="flex-1">
                    <div class="relative group">
                        ...
                        <input type="text" 
                               ...
                               class="w-full bg-[#111827] border-2 border-[#374151] rounded-xl pl-11 pr-4 py-2.5 text-[#E5E7EB] placeholder:text-[#6B7280] focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#3B82F6]/30 transition-all duration-300">
                    </div>
                </div>
                ...
                <div class="w-full md:w-48">
                    <div class="relative group">
                        <select id="topicFilterSelect" 
                                class="w-full bg-[#111827] border-2 border-[#374151] rounded-xl px-4 py-2.5 text-[#E5E7EB] appearance-none cursor-pointer focus:outline-none focus:border-[#3B82F6] transition-all duration-300">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button id="clearFiltersBtn" class="text-xs text-red-400 hover:text-red-500 transition-colors hidden">
                        Clear all
                    </button>
                </div>
            </div>
            ...
            <!-- Filter Stats -->
            <div id="filterStats" class="mt-4 pt-3 border-t border-[#374151] text-xs text-[#6B7280] flex justify-between items-center">
                <div class="flex items-center gap-2">
                    <span class="w-1.5 h-1.5 bg-[#3B82F6] rounded-full"></span>
                    <span id="resultCount">0</span>
                    <span>notes found</span>
                </div>
                <div class="flex gap-3">
                    <button id="selectAllBtn" class="hover:text-[#3B82F6] transition-colors">Select All</button>
                    <button id="bulkDeleteBtn" class="text-red-400 hover:text-red-500 transition-colors hidden">Delete Selected</button>
                </div>
            </div>
        </div>
    `;
...
export function setupSavedNoteFilter(notes, onFilterChange, onSortChange, onSearch, onBulkDelete) {
    ...
    if (selectAllBtn) {
        selectAllBtn.addEventListener('click', () => {
            const noteCards = document.querySelectorAll('.saved-note-card');
            noteCards.forEach(card => {
                const id = parseInt(card.dataset.noteId);
                if (id) {
                    selectedNotes.add(id);
                    card.classList.add('ring-2', 'ring-[#3B82F6]', 'bg-[#1F2937]/50');
                }
            });
    ...
    function updateActiveFilters() {
        ...
        if (resultCount && onFilterChange) {
            const visibleNotes = document.querySelectorAll('.saved-note-card[style*="display: block"], .saved-note-card:not([style*="display: none"])');
            resultCount.textContent = visibleNotes.length;
        }
    }
    
    function updateSelectionCount() {
        if (bulkDeleteBtn) {
            bulkDeleteBtn.innerHTML = `Delete Selected (${selectedNotes.size})`;
        }
    }
